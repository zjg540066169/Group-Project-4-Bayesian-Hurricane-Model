...
regularMHstep <- function(hurrican, a, logp, pars){
  new_pars = list()
  new_pars$beta = pars$beta + 2 * (runif(length(pars$beta)) - 0.5) * a
  new_pars$rho = pars$rho + 2 * (runif(length(pars$rho)) - 0.5) * a
  new_pars$inv_cov = pars$inv_cov + 2 * (matrix(runif(length(pars$inv_cov)), nrow(pars$inv_cov)) - 0.5) * a
  
  
  if(log(runif(1)) < log_posterior(hurrican, new_pars) - log_posterior(hurrican,pars))
    return(new_pars)
  else
    return(pars)
}
...
```{r}
MCMC = function(hurrican, num_samples = 10, sample_gap = 10, a = 10, pars_init, burn_in_init = 1000, burn_in_test = 300){
    if (is.null(k))
      k = burn_in_add
    ...
    while(T){
      ...
      for (i in 1:burn_in_test * sample_gap) {
        
        pars <- regularMHstep(hurrican, a, logp = log_posterior, pars = pars)
        if(i %% sample_gap == 0)
          chain_mcmc[i %/% sample_gap,] = pars$beta
      }
...
pars = list(
    beta = rep(0, 21),
    inv_cov = matrix(rep(0, 9), nrow = 3),
    rho = rep(0, 3)
)

chain_beta = MCMC(hurrican = hurrican, pars_init = pars, burn_in_init = 10000)
