...
```{r}
MCMC = function(hurrican, num_samples = 10, sample_gap = 1, a, pars_init, burn_in_init = 1000, burn_in_test = 300){
    ...
    while(T){
      ...
      if (chain_test_num >= 10){
        if(gelman.diag(chain_beta_test)$mpsrf < 4){
          print("burn in over")
          break
        }
...
chain_samples = MCMC(hurrican = hurrican, pars_init = pars, num_samples = 1000, sample_gap = 30, burn_in_init = 3000, a = 2e-1)
